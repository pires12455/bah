...
async function starts() {
	...
	client.on('connecting', () => {
		start('2', 'ta Conectando...')
	})
	client.on('open', () => {
		success('2', 'Conectado meu caro')
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'calma ae amig�o aguarde um pouco...',
				success: '✔️ Listo ✔️',
				...
				only: {
					group: '[❗] Este comando � apenas para grupos! � � ',
					ownerG: '[❗] Este comando s� pode ser usado por um administrador de grupo! � � ',
					ownerB: '[❗] Este comando � usado apenas pelo PIRES! � � ',
...
`,
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '556996103915@s.whatsapp.net'
                        /******ApiKey Input******/
               ...
               break
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Ok pa, aposentado da posi��o de administrador @${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'Aqui est� o n�mero do meu dono, salve mais tarde ',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
                ...
                break
                   ...
                   break
				case 'wiki':
					if (args.length < 1) return reply('Insira a palavra-chave ')
					tels = body.slice(6)	
                                        if (!isUser) return reply(mess.only.daftarB)				
				...
				case 'wikien':
					...
					break				
				case 'ytmp3':
					if (args.length < 1) return reply('Onde fica o url?')
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'blocklist':
                   ...
                   case 'chatlist':
					client.updatePresence(from, Presence.composing)  
					teks = 'Esta � a lista de n�meros de bate-papo:\n'
				...
				case 'chord':
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Onde est� a carta?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Eu sou um bot, weon ')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('O destino n�o p�de ser adicionado, talvez por ser privado ')
					}
					break
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recebido, tchau pa � ��: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
			...
			case 'closegc':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply('� Falha no momento da convers�o ${tipe} para adesivo ')
							})
							.on('end', function () {
								console.log('Finish')
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Eu n�o o contenho '})
						fs.unlinkSync(ran)
					})
					break
                	case 'tomp3':
                	client.updatePresence(from, Presence.composing) 
                        if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('� � Apenas v�deos � � ')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('� � N�o foi poss�vel converter o v�deo para mp3 � � ')
						buffer = fs.readFileSync(ran)
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					if (args.length < 1) return reply(`Incorreta \nComando : ${prefix}daftar Nome | Idade \nComando : ${prefix}daftar V�rt�x|25`)
					var reg = body.slice(8)
					var jeneng = reg.split("|")[0];
					var umure = reg.split("|")[1];
						user.push(sender)
						fs.writeFileSync('./database/json/user.json', JSON.stringify(user))
						client.sendMessage(from, `\`\`\`Registrado con exito flaco 👏😎 DNI: Socio De La Oscu Army 🤑\`\`\`\n\n\`\`\`Dia ${date} ${time}\`\`\`\n\`\`\`[Usuario]: ${jeneng}\`\`\`\n\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Alto Gay]: ${umure}\`\`\`\n\`\`\`Para usar el bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`enviar ${prefix}help\`\`\`\n\`\`\`\nTotal de usuários ${user.length}\`\`\``, text, {quoted: mek})
					break
                                case 'welcome':
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
					...
					if (Number(args[0]) === 1) {
						...
						reply('� SUCESSO - a fun��o de boas-vindas neste grupo est� ativada ')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, disable)
						fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
						reply('� � SUCESSO � a funç� � o de boas-vindas neste grupo está desabilitada ')
					} else {
						reply('Digite 1 para ativar, 0 para desativar a fun��o ')
					}
                                        break
                                case 'fakta':
					fakta = body.slice(1)
				...
				case 'firetext':
					if (args.length < 1) return reply(mess.blank)
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
					if (tels.ength > 10) return reply('O texto � muito longo, at� 9 caracteres ')
					reply(mess.wait)
				...
				case 'logowolf':
					...
					if (Number(args[0]) === 1) {
						...
						reply('� � SUCESSO � A funç� � o NSFW está habilitada neste grupo ')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply('� � SUCESSO � fun��o NSFW est� desabilitada neste grupo ')
					} else {
				...
				case 'logowolf2':
					...
					break
                case 'truth':
					const trut =['Verdade ou desafio??? Mas que galo de comando ']
            ...
            break
				...
				if (!isUser) return reply(mess.only.daftarB)
			       ...
			       reply(anu.result.jadwalTV)
					...
					break
				case 'ttp':
					if (args.length < 1) return reply('E o texto magro?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
					...
					break
				case 'simi':
					if (args.length < 1) return reply('Cadela, cad� o texto? ')
					teks = body.slice(5)
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[WARN]','red'), ' olk man Comando n�o registrado de ', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
